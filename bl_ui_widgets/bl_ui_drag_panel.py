    ...
    def child_widget_focused(self, x, y):
        for widget in self.widgets:
            if not widget.is_in_rect(x, y):
                continue
            # Smooth-scroll positions buffer grid widgets (asset thumbnails)
            # just outside the visible bar for scroll animation. They are
            # clipped, so a buffer widget overlapping a resize edge (which lies
            # at/beyond the panel border) must not count as a focused child -
            # otherwise edge resizing dies once the bar has buffer rows below it.
            if getattr(widget, "_is_grid_widget", False) and not self.is_in_rect(x, y):
                continue
            return True
        return False
