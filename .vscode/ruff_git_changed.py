#!/usr/bin/env python3
"""
Run Ruff checks on Python files changed by the current branch (commits) relative to its base.
...
- Filter to .py files and run: ruff check <files>

No script arguments by design.
"""
...
def _detect_origin_head() -> Optional[str]:
    try:
        ref = _run_git(["symbolic-ref", "--quiet", "--short", "refs/remotes/origin/HEAD"])
        return ref or None
    except Exception:
        return None
...
def get_git_changed_files_branch() -> List[str]:
    """Get Python files changed by the branch relative to merge-base(base, HEAD)."""
    try:
        base = _pick_default_base()
        mb = _merge_base(base, "HEAD")
        diff_range = f"{mb}..HEAD"
        out = _run_git(["diff", "--name-only", "--diff-filter=ACMRT", diff_range], check=True)
        return [f for f in (out.splitlines() if out else []) if f.endswith(".py")]
...
def main() -> int:
    ...
    ruff_args = ["ruff", "check", *changed_files]
    print(f"\nRunning: {' '.join(ruff_args)}")

    try:
        result = subprocess.run(ruff_args, check=False)
        return result.returncode
    except FileNotFoundError:
        print("Ruff not found. Make sure it's installed and in PATH.")
        return 1
