...
def add_error_report(task_id = '0', text='', timeout=5):
  tasks[task_id] = {
      'type': 'error_report',
      'text': report_text,
      'time' : 10,
...
def report_download_progress(task_id = '0', progress = 0):
  tasks[task_id] = {
...
async def get_download_url(data):#asset_data, scene_id, api_key, tcom=None, resolution='blend'):
    ...
    add_error_report(task_id = data['asset_data']['id'],text = report_text, timeout = 10)
    return False


# def get_download_dirs(data):
...
# 
#     dirs = []
#     if data['PREFS']['directory_behaviour'] in ('BOTH', 'GLOBAL'):
#         ddir = data['PREFS']['global_dir']
#         if ddir.startswith('//'):
#             ddir = os.path.abspath(ddir)
#         if not os.path.exists(ddir):
#             os.makedirs(ddir)
# 
#         subd = subdmapping[data['asset_data']['type']]
#         subdir = os.path.join(ddir, subd)
#         if not os.path.exists(subdir):
#             os.makedirs(subdir)
#         dirs.append(subdir)
#     if (
#             data['PREFS']['directory_behaviour'] == 'BOTH' or data['PREFS'][
...
#         ddir = data['PREFS']['project_subdir']
#         if ddir.startswith('//'):
#             ddir = os.path.abspath(ddir)
#             if not os.path.exists(ddir):
#                 os.makedirs(ddir)
    ...
    if res_file.get('url') is not None:
        ...
        for d in dirs:
            ...
            if sys.platform == 'win32' and len(asset_folder_path) > windows_path_limit:
                add_error_report(task_id=data['asset_data']['id'],
...
class DownloadAsset(web.View):
  ...
  async def doDownload(self, data):
    ...
    # not sure why this was here, since the original thread shouldn't be started a
    ...
    with open(file_name, "wb") as f:
      ...
      if total_length is None:  # no content length header
        ...
        for rdata in response.iter_content(chunk_size=4096 * 32):  # crashed here... why? investigate:
          ...
          print(data['asset_data']['assetBaseId'])
          print(progress)
          report_download_progress(task_id = data['asset_data']['assetBaseId'],
                              progress = progress)
          f.write(rdata)
          # if self.stopped():
          #   bk_logger.debug('stopping download: ' + asset_data['name'])
