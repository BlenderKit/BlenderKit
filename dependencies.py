...
def ensure_preinstalled_deps_copied():
    ...
    if not bundled_version_is_correct():
        ...
        bk_logger.info(
            f"Skipping dependencies copy: bundled for python {bundled}, running on python {sys.version}"
        )
        return
...
def get_deps_directory_path() -> str:
    ...
    addon_version = (
        f"{global_vars.VERSION[0]}-{global_vars.VERSION[1]}-{global_vars.VERSION[2]}"
    )
    blender_python_version = f"{sys.version_info.major}-{sys.version_info.minor}"
    install_path = path.join(
        daemon_directory, "dependencies", addon_version, blender_python_version
    )
...
def ensure_deps():
    ...
    for x in range(2):
        ...
        try:
            ...
            exception = e
            bk_logger.warning(
                f"Failed to import aiohttp.web and aiohttp.web_request: {e}"
            )
            install_dependencies()
            continue
...
def install_dependencies():
    ...
    command = [
        ...
        "--upgrade",
        "-t",
        get_installed_deps_path(),
        "-r",
        requirements,
    ]
    result = subprocess.run(command, env=env, capture_output=True, text=True)
    bk_logger.warning(
        f"AIOHTTP INSTALLATION:\ncommand {command} exited: {result.returncode},\nstdout: {result.stdout},\nstderr: {result.stderr}"
    )
    ...
    bk_logger.warning(
        "Install from requirements.txt failed, trying with unconstrained versions..."
    )
    command = [
        sys.executable,
        "-m",
