        ...
        if task.get("downloaders"):
            for d in task["downloaders"]:
                loc = view3d_utils.location_3d_to_region_2d(
                    bpy.context.region, bpy.context.space_data.region_3d, d["location"]
                )
...
class AssetDragOperator(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if (
            event.type == "ESC"
            or not mouse_in_region(context.region, self.mouse_x, self.mouse_y)
        ...
        if event.type in {"RIGHTMOUSE", "ESC"} or not mouse_in_region(
