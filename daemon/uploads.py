...
async def do_upload(request: web.Request, task: tasks.Task):
  ...
  ok, metadata_response = await upload_metadata(session, task)
  if not ok:
    logging.error('Metadata upload failed')
    return
  logging.warning('Metadata upload was successfull')

  ok, files = await pack_blend_file(task, metadata_response)
  if not ok:
    logging.error('Asset packing failed')
    return
  logging.warning('Assets packed sucessfully')

  ok = await upload_asset_data(session, task, files, metadata_response)
  if not ok:
    logging.error('Asset upload failed')
    return
...
async def upload_metadata(session: ClientSession, task: tasks.Task):
  ...
  if export_data['assetBaseId'] == '':
    try:
      ...
      logging.error(str(e))
      task.error(f'Metadata upload failed.')
      return False, None
    return True, metadata_response
...
async def pack_blend_file(task: tasks.Task, metadata_response):
  ...
  if 'MAINFILE' in upload_set:
    if upload_data['assetType'] == 'hdr':
      ...
      task.change_progress(5, 'preparing scene - running blender instance')
      ...
      if process.returncode != 0:
        task.error("File packing script failed with non zero return code")
        return False, None
  ...
  if 'MAINFILE' in upload_set:
    ...
    if not os.path.exists(fpath):
      task.error("File packing failed, please try manual packing first")
      return False, None
    
  return True, files


async def upload_asset_data(session: ClientSession, task: tasks.Task, files: list, metadata_response: dict):
  ...
  for file in files:
    ...
    with open(file['file_path'], 'rb') as binary_file:
      ...
      if 250 > response.status > 199: #WHY?
        ...
        response = await session.post(upload_done_url, headers=headers)
      else:
  ...
  if uploaded:
    ...
    if 'MAINFILE' not in upload_set:
      if metadata_response.get('verificationStatus') in ('on_hold','deleted','rejected'):
    ...
    if set_uploaded_status:
      ...
      if response.status != 200:
        logging.warning('failed to confirm the upload and push is to state "uploaded"')
