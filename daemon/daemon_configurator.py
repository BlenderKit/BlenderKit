...
async def get_info():
    text = f"\n\nCA_FILE: {CA_FILE}\n"
    text += f"certifi.where(): {certifi.where()}\n"
    text += f"ssl.get_default_verify_paths(): {ssl.get_default_verify_paths()}\n"
    text += f"ssl.OPENSSL_VERSION: {ssl.OPENSSL_VERSION}\n"
    text += f"ssl.OPENSSL_VERSION_INFO: {ssl.OPENSSL_VERSION_INFO}\n"
    text += f"blenderkit.com cert: {ssl.get_server_certificate(('blenderkit.com', '443'))}\n"
    return text
...
async def get_connectors():
    ...
    ssl_contexts = {
        ...
        context5a.__doc__: await context5a(),
...
async def context5a():
    """default context(blenderkit-com-chain.pem) PEM"""
