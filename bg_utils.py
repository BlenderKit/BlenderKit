...
def upload_file(upload_data, f):
    ...
    for _ in range(0, 5):
        try:
            upload_response = daemon_lib.blocking_file_upload(upload['s3UploadUrl'], f['file_path'])
            status_code = upload_response.status_code
            if 250 > status_code > 199:
                upload_done_url = paths.BLENDERKIT_API + '/uploads_s3/' + upload['id'] + '/upload-file/'
                upload_response = daemon_lib.blocking_request(upload_done_url, 'POST', headers, timeout=(1,10))
                reports.add_report(f"Finished file upload: {os.path.basename(f['file_path'])}")
                return True
            message = f"Upload of {f['type']} {os.path.basename(f['file_path'])} failed ({status_code})"
            reports.add_report(message)
        except Exception as e:
            reports.add_report(f"Upload of {f['type']} {os.path.basename(f['file_path'])} failed, err:{e}")
            sleep(1)
    return False
...
def download_asset_file(asset_data, resolution='blend', api_key=''):
    ...
    with open(file_name, "wb") as f:
        ...
        response = daemon_lib.blocking_request('GET', res_file_info['url'], headers=headers)
